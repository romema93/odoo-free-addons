odoo.define('pdf_report_options.report', function (require) {
    ...
    ActionManager.include({
        _showDialogPdfOption: function (action, options) {
            ...
            var option_default = !!action.id;
            var $dialog = new Dialog(self, {
                title: _t("What do you want to do?"),
                size: 'medium',
                $content: QWeb.render('print_attachment_options.button_options', {'option_default': option_default}),
                buttons: [{text: _t('Close'), click: btnClose}]
            }).open();
            $dialog.opened().then(function () {
                $dialog.$('#btn-default-option').click(function (e) {
                    e.preventDefault();
                    self.doAction({
